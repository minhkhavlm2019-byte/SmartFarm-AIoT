...
class ActionType(str, enum.Enum):
    ...
    LIGHT_ON = "LIGHT_ON"     # Bật đèn (Bổ sung cho đầy đủ logic)
    LIGHT_OFF = "LIGHT_OFF"   # Tắt đèn
    SYSTEM_REBOOT = "SYSTEM_REBOOT"           # Bị Watchdog ép reset
    CONNECTION_LOST = "CONNECTION_LOST"       # Rớt mạng (Heartbeat fail)
    CONNECTION_RESTORED = "CONNECTION_RESTORED" # Có mạng lại
    SENSOR_DATA = "SENSOR_DATA"               # Log nhận dữ liệu
...
class TriggerSource(str, enum.Enum):
    ...
    SYSTEM = "SYSTEM"  # Dùng cho các lỗi mạng, watchdog tự sinh ra
...
class User(Base):
    ...
    # --- QUAN HỆ CẬP NHẬT ---
    # 1 User có thể sở hữu nhiều zone (Admin)
    zones_owned = relationship("Zone", foreign_keys="[Zone.owner_id]", back_populates="owner")
    
    # 1 User (Tech) có thể quản lý kỹ thuật nhiều zone
    zones_managed = relationship("Zone", foreign_keys="[Zone.technician_id]", back_populates="technician")
    
    # 1 User (Farmer) có thể canh tác ở nhiều zone
    zones_farmed = relationship("Zone", foreign_keys="[Zone.farmer_id]", back_populates="farmer")
...
class Zone(Base):
    ...
    # --- PHÂN QUYỀN CỤ THỂ ---
    # Người tạo/Sở hữu (Thường là Admin)
    owner_id = Column(Integer, ForeignKey("users.user_id"), nullable=True)
    
    # Kỹ thuật viên phụ trách (Chịu trách nhiệm cấu hình, sửa lỗi)
    technician_id = Column(Integer, ForeignKey("users.user_id"), nullable=True)
    
    # Nông dân canh tác (Chịu trách nhiệm giám sát, thu hoạch)
    farmer_id = Column(Integer, ForeignKey("users.user_id"), nullable=True)
    
    # --- QUAN HỆ ---
    owner = relationship("User", foreign_keys=[owner_id], back_populates="zones_owned")
    technician = relationship("User", foreign_keys=[technician_id], back_populates="zones_managed")
    farmer = relationship("User", foreign_keys=[farmer_id], back_populates="zones_farmed")
...
class ZoneSetting(Base):
    ...
    # Chế độ hoạt động (Thêm vào để lưu trạng thái Auto/Manual của Zone)
    mode = Column(String(20), default="MANUAL")     # AUTO hoặc MANUAL
...
class Device(Base):
    ...
    pump_state = Column(Boolean, default=False)  # Trạng thái máy bơm
    light_state = Column(Boolean, default=False) # Trạng thái đèn
    mist_state = Column(Boolean, default=False)  # Trạng thái phun sương
...
class ActionLog(Base):
    ...
    level = Column(Enum(LogLevel), default=LogLevel.INFO) # Phân loại ERROR, WARN, INFO
    payload = Column(Text, nullable=True) # Lưu chuỗi JSON (chứa thông tin chi tiết)
